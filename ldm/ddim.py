...
class DDIMSampler:
    ...
    def encode(
        ...
        for i in tqdm(range(num_steps), desc='Encoding x0', disable=not progress):
            ...
            if return_intermediates and i % (num_steps // n_intermediates) == 0 and i < num_steps - 1:
                intermediates.append(x_next.cpu())
                inter_steps.append(i)
            elif return_intermediates and i >= num_steps - 2:
                intermediates.append(x_next.cpu())
                inter_steps.append(i)
        
        out = {'x_encoded': x_next.cpu(), 'intermediate_steps': inter_steps}
    ...
    def decode(
        ...
        for i, step in enumerate(iterator):
            ...
            if return_intermediates:
                intermediates.append(x_dec.cpu())
                inter_steps.append(step)

        out = {'x_decoded': x_dec.cpu(), 'intermediate_steps': inter_steps}
        if return_intermediates:
            out.update({'intermediates': intermediates})
        return x_dec, out
