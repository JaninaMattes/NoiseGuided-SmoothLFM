...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        print("[OpenAIUNet] Creating Model")
    ...
    def load_from_ckpt(self, ckpt_path):
        ...
        assert input_ch >= 4 and input_ch // 4 * 4 == input_ch, (
            "Input channels must be at a multiplier 4 to load from SD ckpt"
        )
        output_ch = self.state_dict()["out.2.weight"].shape[0]
        assert output_ch >= 4 and output_ch // 4 * 4 == output_ch, (
            "Output channels must be at a multiplier 4 to load from SD ckpt"
        )
        ...
        if input_ch > 4:
            ...
            sd_["input_blocks.0.0.weight"] = sd_["input_blocks.0.0.weight"].repeat(
                1, scale, 1, 1
            )
