...
def center_crop_arr(pil_img, img_size):
    ...
    while min(*pil_img.size) >= 2 * img_size:
        pil_img = pil_img.resize(tuple(x // 2 for x in pil_img.size), resample=img.BOX)
...
class DCTEmbedder:
    def __init__(
        self, norm=None, remove_low_freq_ratio=0.5, height=32, width=32, block_size=8
    ):
    ...
    def encode(self, x):
        ...
        x_blocks = self.split_into_blocks(
            x, self.block_size
        )  # (B, 4, num_blocks, 8, 8)
        B_blocks = x_blocks.view(
        ...
        dct_tokens = self.remove_low_frequencies(
            B_dct_blocks
        )  # (B * num_blocks*4, 8, 8)
        return dct_tokens
    ...
    def decode(self, tokens):
        ...
        x_recon = self.combine_blocks(
            x_blocks, self.block_size, self.height, self.width
        )
        return x_recon
    ...
    def remove_low_frequencies(self, x_dct):
        ...
        threshold = (
            torch.topk(mag_flat, threshold_idx, dim=-1, largest=False)
            .values[:, -1]
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
    ...
    def split_into_blocks(self, x, block_sz):
        B, C, H, W = x.shape
        x = x.unfold(2, block_sz, block_sz).unfold(
            3, block_sz, block_sz
        )  # (B, C, H//bs, W//bs, bs, bs)
        x = x.contiguous().view(
            B, C, -1, block_sz, block_sz
        )  # (B, C, num_blocks, bs, bs)
        return x
...
class DCTPatchEmbedder(nn.Module):
    ...
    def forward(self, x):
        ...
        dct_patches = dct_patches.reshape(
            B, -1, C, self.patch_size * self.patch_size
        )  # (B, N, C, p*p)
...
class IDCTPatchDecoder(nn.Module):
    ...
    def __init__(
        ...
        in_dim = out_channels * (
            high_freqs if high_freqs is not None else patch_size * patch_size
        )
    ...
    def forward(self, x):
        ...
        if self.low2high_order is not None:
            inv_order = torch.argsort(
                torch.tensor(self.low2high_order, device=x.device)
            )
...
class DCT_4YCbCr:
    def __init__(
        ...
        self.y_index = [
            ...
            if i not in self.cb_index and i not in self.cr_index
        ]
    ...
    def __getitem__(self, idx):
        ...
        cb_downsampled = cv2.resize(
            img_cb,
            (img_cb.shape[1] // 2, img_cb.shape[0] // 2),
            interpolation=cv2.INTER_LINEAR,
        )
        cr_downsampled = cv2.resize(
            img_cr,
            (img_cr.shape[1] // 2, img_cr.shape[0] // 2),
            interpolation=cv2.INTER_LINEAR,
        )

        # Step 2: Split the Y, Cb, and Cr components into 4x4 blocks
        y_blocks = split_into_blocks(
            img_y, self.block_sz
        )  # Y component, (64, 64) --> (256, 4, 4)
        cb_blocks = split_into_blocks(
            cb_downsampled, self.block_sz
        )  # Cb component, (32, 32) --> (64, 4, 4)
        cr_blocks = split_into_blocks(
            cr_downsampled, self.block_sz
        )  # Cr component, (32, 32) --> (64, 4, 4)
        ...
        DCT_blocks = np.array(DCT_blocks).reshape(
            -1, 6, self.block_sz * self.block_sz
        )  # (64, 6, 4, 4) --> (64, 6, 16)
        ...
        DCT_blocks = DCT_blocks[
            :, :, : self.low_freqs
        ]  # (64, 6, 16) --> (64, 6, low_freq_coe)

        # numpy to torch
        DCT_blocks = torch.from_numpy(DCT_blocks).reshape(
            self.tokens, -1
        )  # (64, 6*low_freq_coe)
