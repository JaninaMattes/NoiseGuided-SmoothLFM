...
def DCT_to_RGB(
    ...
    DCT = DCT[
        ..., reverse_order
    ]  # convert the low to high freq order back to 8*8 order
    ...
    DCT_Cb = DCT_Cb.reshape(
        num_cb_blocks, block_sz, block_sz
    )  # (64, 16) --> (64, 4, 4)
    DCT_Cr = DCT_Cr.reshape(
        num_cb_blocks, block_sz, block_sz
    )  # (64, 16) --> (64, 4, 4)
    ...
    cb_reconstructed = combine_blocks(
        idct_cb_blocks, int(height / 2), int(width / 2), block_sz
    )
    cr_reconstructed = combine_blocks(
        idct_cr_blocks, int(height / 2), int(width / 2), block_sz
    )

    # Upsample Cb and Cr to original size
    cb_upsampled = cv2.resize(
        cb_reconstructed, (width, height), interpolation=cv2.INTER_LINEAR
    )
    cr_upsampled = cv2.resize(
        cr_reconstructed, (width, height), interpolation=cv2.INTER_LINEAR
    )
    ...
    G = (
        y_reconstructed
        - 0.344136 * (cb_upsampled - 128)
        - 0.714136 * (cr_upsampled - 128)
    )
...
def DCTsamples_to_grid_image(
    ...
    rgb_imgs = []
    for sample in samples:
        rgb_reconstructed = DCT_to_RGB(
            sample, tokens, low_freqs, block_sz, reverse_order, resolution, Y_bound
        )
    ...
    for i in range(grid_sz):
        for j in range(grid_sz):
            ...
            if idx < rgb_imgs.shape[0]:
                grid_image[
                    i * img_sz : (i + 1) * img_sz, j * img_sz : (j + 1) * img_sz, :
                ] = rgb_imgs[idx]
        ...
        if accelerator.is_main_process:
            for sample in samples:
                ...
                cv2.imwrite(
                    ...
                    cv2.cvtColor(rgb_reconstructed, cv2.COLOR_RGB2BGR),
                )
                idx += 1
