...
def initialize_weights(model: nn.Module, init_type: str = "kaiming", **kwargs) -> None:
    ...
    """
    init_funcs = {"normal": normal_init, "xavier": xavier_init, "kaiming": kaiming_init}

    if init_type not in init_funcs:
        raise ValueError(
            f"Initialization method {init_type} not supported. "
...
def normal_init(
    module: nn.Module, mean: float = 0.0, std: float = 1.0, bias: bool = True
) -> None:
...
def xavier_init(module: nn.Module, gain: float = 1.0, bias: bool = True) -> None:
