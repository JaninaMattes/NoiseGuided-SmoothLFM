...
def run_pca_eval(
    ...
    label_map = {int(original): int(idx) for idx, original in enumerate(sorted(unique_labels))}
    ...
    print(f"[INFO] Saved label_map and inverse_label_map.")
    ...
    print(f"[INFO] Linear probe training completed.")
    ...
    multi_layer_probe = TwoLayerProbe(input_dim=num_components, hidden_dim=128, num_classes=num_classes)
    ...
    plot_validation_curve(df_metrics_multi, save_path=plot_path_multi, source_timestep=source_timestep, target_timestep=target_timestep, beta=beta)
    print(f"[INFO] Multi-layer probe training completed.")
    ...
    return df_metrics, df_metrics_multi, pca_latents, all_labels_mapped, pca.components_, pca.explained_variance_ratio_
...
if __name__ == "__main__":
    ...
    model_configs_v0 = [
        ...
        {"name": "Beta00x00x_01b",  "beta": 0.1,  "source_ts": 0.20, "target_ts": 0.20, "ckpt": './logs_dir/imnet256/beta-vae-skipViT-b-2/imagenet256_hdf5_v2/0.20x-0.20x-0.1b/2025-06-18/29842/V2/2025-06-18/29842/checkpoints/last.ckpt' },  # Open
        # beta: 1.0
    ...
    all_config_groups = [
        ...
        {"group_name": "Beta0.1_ReconstructionTask_models", "configs": model_configs_v4},
    ...
    for config_group in all_config_groups:
        ...
        for config in configs:
            ...
            print(f"[INFO] Metrics for {model_tag} saved to: results/{project_name}/{model_name}/linear_probe_metrics.csv")
            ...
            if not timestamp_dirs:
                print(f"[WARN] No results found for model: {model_tag}, skipping for UMAP grid.")
                continue

            latest_dir = max(timestamp_dirs, key=os.path.getmtime)

            latents_path = latest_dir / "pca_latents.npy"
            labels_path  = latest_dir / "pca_labels_original.npy"  # or "pca_labels_mapped.npy" if desired

            if not latents_path.exists() or not labels_path.exists():
                print(f"[WARN] Missing PCA files for model: {model_tag}, skipping UMAP grid.")
                continue
        ...
        if all_metrics:
            ...
            for model_tag, color in zip(combined_df["Model"].unique(), palette):
                ...
                plt.plot(model_df["Epoch"], model_df["Val_Accuracy"], marker='o', linewidth=2, label=model_tag, color=color)

            plt.title(f"Validation Accuracy Comparison — {group_name}", fontsize=18, pad=20)
        ...
        if all_multi_metrics:
            ...
            plt.figure(figsize=(14, 8))
            sns.set(style="whitegrid", font="serif")
            palette = sns.color_palette("crest", n_colors=len(combined_multi_df["Model"].unique()))
            ...
            plt.title(f"Validation Accuracy Comparison (Multi-Layer) — {group_name}", fontsize=18, pad=20)
            plt.xlabel("Epoch", fontsize=15)
            plt.ylabel("Validation Accuracy", fontsize=15)
            plt.ylim(0, 0.5)
            plt.legend(title="Model", fontsize=10, loc="upper right")
            ...
            combined_multi_plot_path = f"combined_validation_curve_multi_{group_name}.png"
        ...
        if model_results:
            ...
            plot_umap_grid(model_results, group_name=group_name, save_path=umap_grid_save_path, 
                           n_neighbors=n_neighbors, min_dist=min_dist, n_components=num_components, max_data_samples=max_umap_samples, random_state=random_state
                           )
