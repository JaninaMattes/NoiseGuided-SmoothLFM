...
class LatentSmoothnessMetricsTracker(nn.Module):
    ...
    def aggregate(self):
        ...
        return {"ppl": np.mean(self.ppl_values), "istd": np.mean(self.istd_values)}
...
def test_smoothness_metrics():
    ...
    for _ in range(B):
        ...
        seq = torch.cat(
            [lerp(alpha, start_img, end_img) for alpha in torch.linspace(0, 1, T)],
            dim=0,
        )
    ...
    print(
        f"Identical: PPL={identical_results['ppl']:.6f}, ISTD={identical_results['istd']:.6f}"
    )
    tracker.reset()
