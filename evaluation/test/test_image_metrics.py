...
def create_dummy_folder(path: str, num_images: int = 16, image_size=(3, 128, 128)):
    ...
    print(
        f"Created dummy folder at {path} with {num_images} images of size {image_size}"
    )
...
def test_torch_fidelity_metrics_large_set(
    fake_dir="tmp/fake", real_dir="tmp/real", num_samples=5000
):
    """
    ...
    assert "precision" in metrics and "recall" in metrics, (
        "Precision/Recall not computed! Increase image count."
    )
    assert metrics["precision"] > 0.0 and metrics["recall"] > 0.0, (
        "Precision/Recall should be greater than 0!"
    )
