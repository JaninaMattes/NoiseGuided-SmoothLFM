...
def traverse_two_latent_dimensions(
    mu, std, n_samples=16, latent_dim=512, dim_1=0, dim_2=1, digit_size=32
):
    ...
    figure = np.zeros(
        (digit_size * n_samples, digit_size * n_samples, 3)
    )  # Initialize with 3 channels for RGB
        ...
        try:
            ...
            for i in range(latents.shape[1]):
                for j in range(i + 1, latents.shape[1]):
                    mi_scores.append(
                        mutual_info_score(
                            discretize(latents[:, i]), discretize(latents[:, j])
                        )
                    )
