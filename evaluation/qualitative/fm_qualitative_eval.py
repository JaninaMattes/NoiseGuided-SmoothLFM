...
def linear_interpolation_grid_with_anchors(
    ...
    with torch.no_grad():
        for i in range(num_pairs):
            ...
            C, H, W = x1.size()[1:]
            noise_x = torch.randn(B, C, H, W, device=device, dtype=torch.float32)
            ...
            sample_kwargs["context"] = interp_context
    ...
    all_rows = torch.cat(samples_rows, dim=0)
...
def traverse_latents_kmeans(
    ...
    real_img = real_img[:3]
    real_img = TF.resize(real_img, [img_h, img_w])
    real_img = real_img.permute(1, 2, 0).detach().cpu().numpy()
    real_img = (real_img - real_img.min()) / (real_img.max() - real_img.min() + 1e-6)
    real_img = (real_img * 255).astype(np.uint8)
...
def generate_samples(
    ...
    with torch.no_grad():
        for i in range(num_chunks):
            ...
            samples = fm_module.model.generate(x=z, **sample_kwargs)
            samples_imgs = fm_module.decode_first_stage(samples)

            fake_images = denorm_tensor(samples_imgs).detach().cpu()
            ...
            del x_chunk, img_chunk, lbl_chunk, context, z, samples_imgs
            torch.cuda.empty_cache()
            gc.collect()
...
def run_eval(
    ...
    samples_dir = base_results_dir / "samples_samples"
