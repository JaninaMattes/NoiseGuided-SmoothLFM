...
def unsharp_mask(img, amount=1.0, threshold=0):
    ...
    if threshold > 0:
        ...
        diff = np.abs(
            img.astype(np.int16) - blurred.astype(np.int16)
        )  # int16 to avoid wraparound
...
def collect_samples(
    ...
    for label in class_labels:
        ...
        if len(latents_list) < samples_per_class:
            print(
                f"[WARNING] Not enough samples for class {label}: got {len(latents_list)}, needed {samples_per_class}. Skipping."
            )
            continue
...
def linear_interpolation_grid_with_evaluation(
    ...
    if not all(
        ...
        print(
            f"[WARNING] Skipping interpolation: Not enough data for {num_pairs} pairs."
        )
        return None
    ...
    for i in tqdm(range(num_pairs), desc="Interpolating pairs"):
        ...
        uc_cond = (
            torch.full(
                (interp_context.size(0),), num_classes, device=device, dtype=torch.long
            )
...
def run_interpolation_and_evaluation(
    ...
    base_results_dir = (
        Path(results_root) / project_name / "Interpolations" / model_name / timestamp
    )
    ...
    for interp_name, (cls_a, cls_b) in interpolation_dict.items():
        try:
            ...
            if latents_1.size(0) < num_pairs or latents_2.size(0) < num_pairs:
                print(
                    f"[WARNING] Skipping pair {interp_name} due to insufficient samples ({latents_1.size(0)}, {latents_2.size(0)})"
                )
                continue
        ...
        print(
            f"[METRICS] Average PPL: {tracker_metrics['ppl']:.8f}, ISTD: {tracker_metrics['istd']:.8f}"
        )
        print(
            f"[METRICS] SSIM: {image_metrics['ssim']:.8f}, PSNR: {image_metrics['psnr']:.8f}, MSE: {image_metrics['mse']:.8f}, "
