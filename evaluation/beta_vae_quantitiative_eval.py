...
class ImageMetricsTracker(nn.Module):
    ...
    def __call__(self, target, pred, noise_target=None, noise_pred=None):
        ...
        print(
            f"Min: {target.min().item():.3f}, Max: {target.max().item():.3f} | Min: {pred.min().item():.3f}, Max: {pred.max().item():.3f}"
        )
...
def interpolate_latents_with_smoothness_eval(
    ...
    for i in tqdm(range(num_pairs), desc="Interpolating Latents", leave=False):
        ...
        interp_context = interpolate_vectors(
            context_z1.to(device),
            context_z2.to(device),
            alpha_vals=alpha_lin_space,
            mode=interp_type,
        )
        ...
        if plot_samples:
            ...
            row_images = (
                denorm_tensor(generated_interpolation * 0.5 + 0.5).detach().cpu()
            )
...
def run_quant_eval(
    ...
    assert len(cfg_scales) == len(ccfg_scales), (
        "cfg_scales and ccfg_scales must have the same length."
    )
    ...
    with open(csv_path, "w") as csv_file:
        ...
        count_interp = 0
        for interp_name, (cls_a, cls_b) in tqdm(
            interpolation_dict.items(), desc="Processing Interpolations"
        ):
            # reset per grid pair
            smoothness_tracker.reset()
            ...
            mean_mdpl, mean_cdpl, mean_mistd, mean_cistd = (
                out["latent_mdpl"],
                out["latent_cdpl"],
                out["latent_mistd"],
                out["latent_cistd"],
...
if __name__ == "__main__":
    ...
    # beta: 2.0
    Beta02x10x_2b = "./logs_dir/imnet256/beta-vae-skipViT-b-2/imagenet256_hdf5_v2/0.20x-1.0x-2.0b/V2/2025-07-06/101646/checkpoints/last.ckpt"  # Open
    ...
    max_samples = (
        1000  # Maximum number of samples to process in total (50k for FID, etc.)
    )
