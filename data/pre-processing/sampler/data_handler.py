...
class NumpyDataHandler:
    ...
    def load_from_numpy(self, base_dir: Path, batch_size: int = 16) -> wds.WebLoader:
        """Load images, labels, and latents from numpy files"""
        loader = WebDataModuleFromNumpy(base_dir=base_dir, batch_size=batch_size)
        return loader
...
class HDF5DatasetManager:
    ...
    def append_to_hdf5(
        ...
        with h5py.File(file_path, "a") as h5_file:
            ...
            for img_path, label_path in tqdm(
                zip(image_paths, label_paths),
                desc="Appending images and labels",
                total=num_samples,
            ):
                if img_path.stem != label_path.stem:
                    print(
            ...
            for timestep_dir in tqdm(latent_timestep_dirs, desc="Appending latents"):
                ...
                for latent_path in tqdm(
                    latent_paths, desc=f"Appending timestep {timestep}", leave=False
                ):
                    try:
                        latent = np.load(latent_path).astype(np.float32)
                    except Exception as e:
...
def save_hdf5_intermediate_latents(
    data_dir, filename, timestep=0.5, group_name="train", timesteps=None
):
