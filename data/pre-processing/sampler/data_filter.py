...
def make_filtered_loader(
    ...
    dset_pipe = (
        ...
        .map(
            lambda sample: keep_only_classes(sample, set(class_labels)),
            handler=wds.warn_and_continue,
        )
    ...
    bs = (
        ...
        else (data.batch_size if train else data.val_batch_size)
    )
...
def visualize_batch(batch, class_names=None, save_path=None, nrow=8):
    ...
    if class_names:
        label_names = [
            class_names[int(l)] if isinstance(class_names, dict) else str(int(l))
            for l in labels[: nrow**2]
        ]
...
if __name__ == "__main__":
    ...
    for batch_idx, batch in enumerate(
        islice(dataloader, start_batch_id, end_batch_id), start=start_batch_id
    ):
