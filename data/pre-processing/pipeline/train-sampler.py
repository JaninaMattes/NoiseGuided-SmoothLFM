...
def img_to_grid(
    img: torch.Tensor, stack: str = "row", split: Optional[int] = 4
) -> torch.Tensor:
...
class SampleProcessor:
    def __init__(
        ...
        # Load models
        self.first_stage = torch.compile(
            AutoencoderKL(ckpt_path=first_stage_ckpt).to(self.device), fullgraph=True
        )
    ...
    def process(self) -> None:
        ...
        for batch_idx, (images, labels) in enumerate(self.dataloader):
            ...
            if batch_idx % self.log_every == 0:
                ...
                show_samples(
                    filtered_intermediates,
                    save_dir=str(self.target_data_dir),
                    prefix=f"{self.group_name}_samples_{batch_idx}",
                )
    ...
    def save_files_to_data_dir(
        self, data: Dict[str, Any], group_name: str = "train"
    ) -> str:
        """
