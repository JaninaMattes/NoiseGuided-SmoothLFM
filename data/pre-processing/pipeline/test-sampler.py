...
def show_samples(
    ...
    save_dir=None,
    prefix="",
    title="One-sided diffusion: $z_t = (1 - t) z_1 + t \epsilon$",
):
    ...
    # [T, B, C, H, W] -> [B, T, C, H, W]
    # shape: [T, B, C, H, W]
    all_steps = torch.stack(list(sorted_intermediates.values()))
...
class SampleProcessor:
    ...
    def encode_second_stage(
        ...
        return self.second_stage.encode(
            latent,
            y=y,
    ...
    def __call__(self):
        ...
        filename = (
            self.hdf5_file_name
            or f"imagenet256-testset-{datetime.datetime.now().strftime('T%H%M%S')}.hdf5"
        )
        ...
        gc.collect()
        print(
            f"Processing completed for {self.type} samples. Data saved to {self.data_dir}/{filename}"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Process test samples and save latent encodings."
    )

    parser.add_argument("--data_dir", type=str, required=True, help="Path to dataset")
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Where to save processed outputs"
    )
    parser.add_argument(
        "--hdf5_file", type=str, default=None, help="Optional HDF5 file name"
    )
    parser.add_argument(
        "--first_stage_ckpt",
        type=str,
        required=True,
        help="Path to first stage checkpoint",
    )
    parser.add_argument(
        "--second_stage_ckpt",
        type=str,
        required=True,
        help="Path to second stage checkpoint",
    )
