...
#SBATCH --job-name=SiT-XL2_05x05x5b_cls_drop_run
...
# ---------------------- Env Setup
cd /export/home/<user-id>/dev/Img-IDM/
source /export/home/<user-id>/miniconda3/bin/activate
...
# ---------------------- Dynamic Parameters
export SOURCE_TIMESTEP=0.50
export BETAVAE_TARGET_TIMESTEP=0.50     # Target for BetaVAE  (1.0: data manifold)
...
export ARGS="experiment=imnet256/sit-xl_context_cond_cfg_v0 \
  ...
  name=imnet256/SiT-XL-2/context_cls_cond_w_dropout/${TIMESTEP_TAG}/${BETAVAE_MODEL_TYPE} \
  devices=1 \
  checkpoint_params.every_n_train_steps=5000 \
  autoencoder=sd_ae \
  ldm_autoencoder=v0/dataset_2/05x05x_0.5b_baseline \
  metrics=img_metrics \
  use_wandb=True"
